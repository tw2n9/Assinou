...
export type AvailabilityRuleInput = {
  ...
  barberHours?: TimeInterval | TimeInterval[] | null;
...
export function buildAvailableSlots(input: AvailabilityRuleInput) {
  ...
  const barberHourRanges = Array.isArray(input.barberHours) ? input.barberHours : [input.barberHours];
  const availableSlots: string[] = [];

  if (input.durationMinutes <= 0 || input.slotIntervalMinutes <= 0) {
    return availableSlots;
  }

  for (const barberHours of barberHourRanges) {
    const start = Math.max(toMinutes(input.businessHours.startsAt), toMinutes(barberHours.startsAt));
    const end = Math.min(toMinutes(input.businessHours.endsAt), toMinutes(barberHours.endsAt));
