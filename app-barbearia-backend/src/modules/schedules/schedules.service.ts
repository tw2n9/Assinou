...
export async function getAvailability(input: AvailabilityInput, databaseQuery: QueryExecutor = query) {
  ...
  const barberResult = await databaseQuery<{ is_active: boolean; default_service_duration_minutes: number | null }>(
    "SELECT is_active, default_service_duration_minutes FROM barbers WHERE id = $1",
    [input.barberId]
  );
  ...
  if (!barberHours.length) return { ...input, availableSlots: [] };
