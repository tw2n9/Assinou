...
export async function getAvailability(input: AvailabilityInput, databaseQuery: QueryExecutor = query) {
  const barberResult = await databaseQuery<{ is_active: boolean; default_service_duration_minutes: number | null; barbershop_id: string }>(
    "SELECT is_active, default_service_duration_minutes, barbershop_id FROM barbers WHERE id = $1",
    [input.barberId]
  );
  ...
  const serviceResult = await databaseQuery<{ duration_minutes: number; is_active: boolean }>(
    "SELECT duration_minutes, is_active FROM services WHERE id = $1 AND barbershop_id = $2",
  ...
  const businessResult = await databaseQuery<{ opens_at: string; closes_at: string }>(
    "SELECT opens_at, closes_at FROM business_hours WHERE barbershop_id = $1 AND weekday = $2 AND is_active = true",
    [barbershopId, weekday]
  );
  const businessHours = businessResult.rows[0];
  if (!businessHours) return { ...input, availableSlots: [] as string[] };
  ...
  const barberHours = barberHoursResult.rows.map((item) => ({
    startsAt: item.starts_at,
    endsAt: item.ends_at
  }));
  if (!barberHours.length) return { ...input, availableSlots: [] as string[] };
  ...
  const bookings = await databaseQuery<{ starts_at: string; ends_at: string }>(
    ...
    [input.barberId, input.date, barbershopId]
  );

  const blocks = await databaseQuery<{ starts_at: string; ends_at: string }>(
    `SELECT starts_at, ends_at FROM schedule_blocks
     WHERE barbershop_id = $3 AND date = $1 AND (barber_id = $2 OR barber_id IS NULL)`,
    [input.date, input.barberId, barbershopId]
  );
  ...
  const settings = await databaseQuery<{ default_slot_interval_minutes: number }>(
    "SELECT default_slot_interval_minutes FROM settings WHERE barbershop_id = $1 ORDER BY created_at LIMIT 1",
    [barbershopId]
  );
  ...
  return {
    ...
    availableSlots: buildAvailableSlots({
      ...
      barberHours,
      busyIntervals,
      durationMinutes: barber.default_service_duration_minutes ?? service.duration_minutes,
...
export async function createScheduleBlock(input: CreateScheduleBlockInput) {
  ...
  const conflictingBookings = await query<{ id: string }>(
    `SELECT id FROM bookings
     WHERE date = $1
       ...
       AND ends_at > $3
     LIMIT 1`,
    [input.date, input.barberId ?? null, input.startsAt, input.endsAt, barbershopId]
  );
  ...
  const existingBlocks = await query<{ barber_id: string | null; starts_at: string; ends_at: string }>(
    ...
    [input.date, input.barberId ?? null, barbershopId]
  );
  ...
  const result = await query(
    `INSERT INTO schedule_blocks (barbershop_id, barber_id, date, starts_at, ends_at, reason)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id, barber_id AS "barberId", date, starts_at AS "startsAt", ends_at AS "endsAt", reason`,
    [barbershopId, input.barberId ?? null, input.date, input.startsAt, input.endsAt, input.reason ?? null]
  );
